...
class Income extends Admin_Controller{
	...
	public function dashboard(){
		...
		$query = "Select sum(`total_price`) as `total_income`, 
		 ...
		 if($total_income){ $this->data['total_income'] = $total_income; }else{ $this->data['total_income'] = 0; }
		 if($online){ $this->data['online'] = $online; }else{ $this->data['online'] = 0; }
		 if($print){ $this->data['print'] = $print; }else{ $this->data['print'] = 0; }
		 if($courier){ $this->data['courier'] = $courier; }else{ $this->data['courier'] = 0; }
		 if($special_charges){ $this->data['special_charges'] = $special_charges; }else{ $this->data['special_charges'] = 0; }
		 if($others){ $this->data['others'] = $others; }else{ $this->data['others'] = 0; }
		 if($courier_count){ $this->data['courier_count'] = $courier_count; }else{ $this->data['courier_count'] = 0; }
